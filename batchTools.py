   ...
   def __init__ (self,baseName,prodName,stepList,targetList,batchSplit,postFix='',usePython=False,useBatchDir=True,wDir=''):
     ...
     for iStep in stepList:
       ...
       for iTarget in targetList:
         ...
         jName  = kStep+'__'+kTarget+postFix
     ...
     for jName in self.jobsList:
       ...
       if 'cern' in os.uname()[1] :
         ...
         #CMSSW=CMSSW_8_0_26
         jFile.write('#@ job_name = '+jName+'\n')
        # jFile.write('#$ -q all.q\n')
         jFile.write('#@ initialdir = .\n')
         jFile.write('#@ output = '+self.subDir+'/'+jName+'.out \n')
         jFile.write('#@ error = '+self.subDir+'/'+jName+'.err \n')
         jFile.write('#@ total_tasks = 1 \n')
         jFile.write('#@ wall_clock_limit = 03:00:00 \n')
         jFile.write('export X509_USER_PROXY=/user/'+os.environ["USER"]+'/.proxy\n')
         jFile.write('module load CMS \n')
       #  jFile.write('export SCRAM_ARCH='+SCRAMARCH+'\n')
       #  jFile.write('source $VO_CMS_SW_DIR/cmsset_default.sh\n')
       #  jFile.write('cmsrel '+CMSSW '\n')
...
# mkdir: cannot create directory `/tmp/piedra/latinos': No such file or directory
         elif 'cloudprv' in os.uname()[1]:
           jFile.write("cd /gpfs/csic_users/jgarciaf/ \n") 
         elif 'login' in os.uname()[1]:
           jFile.write("cd /gpfs/csic_users/jgarciaf/ \n") 
         else:
           jFile.write('cd - \n')
   ...
   def Sub(self,queue='8nh',IiheWallTime='168:00:00'): 
     ...
     for jName in self.jobsList:
	  ...
	  queue='cms'
          ...
          jobid=os.system('mnsubmit --reservation=master_2017 '+jobFile + '| awk \'{print $4}\' '+' >' +jidFile)
        elif "pi.infn.it" in socket.getfqdn():
          queue="cms"
...
#         print jidFile
          if 'iihe' in os.uname()[1] :
            ...
            iStat = os.popen('cat '+jidFile+' | awk -F\'.\' \'{print $1}\' | xargs -n 1 qstat | grep Latino | awk \'{print $5}\' ').read() # To be done for altamira "login"
            if 'Q' in iStat : Pend[iStep]+=1
            else: Runn[iStep]+=1
          else:
