...
void PrintYields(float cut)
{
  ...
  float ndata   = 0.0; if(_systematic == "nominal") ndata = GetYield("01_Data", cut);
  ...
  float yield00 = 0.0; if(_systematic == "nominal") yield00 = GetYield("00_Fakes", cut);
  float yield02 = GetYield("02_WZTo3LNu" , cut);
  float yield03 = GetYield("03_ZZ"       , cut);
  float yield04 = GetYield("04_TTTo2L2Nu", cut);
  float yield05 = GetYield("05_ST"       , cut);
  float yield06 = GetYield("06_WW"       , cut);
  float yield07 = GetYield("07_ZJets"    , cut);
  float yield09 = GetYield("09_TTV"      , cut);
  float yield11 = GetYield("11_Wg"       , cut);
  float yield14 = GetYield("14_HZ"       , cut);
